...
public class MonsterInventoryController : MonoBehaviour
{
    ...
    [SerializeField] private previewAnimationControl previewAnimationControl;

    void Start()
    {
        refreshInventory();
        createPreview(monsterInfoManager.GetMonsterInfoWithType(MonsterType.Bat), MonsterType.Bat);
    }
    ...
    private void refreshInventory()
    {
        ...
        foreach (MonsterType monsterType in Enum.GetValues(typeof(MonsterType)))
        {
            ...
            //set image
            temp.transform.GetChild(1).GetComponent<Button>().image.sprite = monsterInfo.GetPreviewImage();
            if(!monsterInfo.IsUnlocked)
            {
                //temp.transform.GetComponent<Image>().color = new Color32(162,162,162,255);
                temp.transform.GetChild(1).GetComponent<Button>().image.color = new Color32(162,162,162,255);
            }
            //set name
            temp.transform.GetChild(1).GetChild(0).GetComponent<TextMeshProUGUI>().text = monsterType.ToString();
            //set preview listener
            MonsterType tempType = monsterType;
            temp.transform.GetChild(1).GetComponent<Button>().onClick.AddListener(() => { createPreview(monsterInfo, tempType); });
    ...
    {
        previewAnimationControl.activatePreview(monsterType);
        //set monster name
